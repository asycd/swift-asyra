...
import { useActionState, useEffect, useRef, useState } from "react";
import { toast } from "sonner";
import { EnterIcon, LoadingIcon, MicIcon } from "@/lib/icons";  // Add MicIcon for the recording button
import { usePlayer } from "@/lib/usePlayer";
import { track } from "@vercel/analytics";
import { useMicVAD, utils } from "@ricky0123/vad-react";
...
export default function Home() {
	const [input, setInput] = useState("");
	const [inputType, setInputType] = useState<'text' | 'voice'>('text'); // State for input type
	const inputRef = useRef<HTMLInputElement>(null);
	const player = usePlayer();

	const vad = useMicVAD({
		startOnLoad: false,  // Do not start recording automatically
		onSpeechEnd: (audio) => {
			player.stop();
			const wav = utils.encodeWAV(audio);
			const blob = new Blob([wav], { type: "audio/wav" });
	...
	function handleFormSubmit(e: React.FormEvent) {
		...
		if (inputType === 'text') {
			...
			vad.start();  // Start voice recording
		}
	}
		...
		<>
			...
			>
				{inputType === 'text' ? (
					...
					<button
						type="button"
						onClick={() => vad.start()}  // Start recording when clicked
						className="p-4 text-neutral-700 hover:text-black dark:text-neutral-300 dark:hover:text-white flex items-center"
